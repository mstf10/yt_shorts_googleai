...
import { Download, Copy, Check, FileText, Code, Terminal, X } from 'lucide-react';
...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'json' | 'markdown' | 'python'>('json');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 space-y-4 shadow-2xl relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-200 p-1 rounded-lg bg-slate-800"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-bold text-slate-100">Export Script & Assets</h3>
        </div>
